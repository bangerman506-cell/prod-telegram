...
def get_cache_stats() -> Dict:
    ...
    """
    try:
        # 1. Get count of active files
        count_query = db.client.table('pikpak_files')\
        ...
        # 2. Get last sync time (most recent created_at or updated_at)
        time_query = db.client.table('pikpak_files')\
            .select('created_at')\
            .eq('is_trash', False)\
        ...
        last_sync = time_query.data[0]['created_at'] if time_query.data else None
        
        # 3. Return combined stats
        return {
            'active_files': count, 
            'trashed_files': 0, # Placeholder if you don't track trashed count
            'total_files': count,
            'total_size_bytes': 0, # Requires DB aggregation function
            'last_sync_time': last_sync
        }
            
    except Exception as e:
        print(f"❌ DB Error (get_cache_stats): {e}", flush=True)
        return {
            'active_files': 0, 
            'trashed_files': 0, 
            'total_files': 0,
